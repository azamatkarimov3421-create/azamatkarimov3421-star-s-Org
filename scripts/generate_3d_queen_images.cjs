  ...
  if (req.method === 'POST' && req.url === '/save') {
    ...
    req.on('end', () => {
      try {
        ...
        console.log('All Queen images saved successfully! Shutting down server...');
        setTimeout(() => {
          server.close();
          process.exit(0);
        }, 1000);
...
<body style="background: transparent; margin: 0; overflow: hidden;">
  ...
  <script type="module">
    ...
    loader.load('/public/models/queen.glb', (gltf) => {
      const results = {};
      ...
      function renderPiece(isWhite, facingDeg) {
        ...
        const fill = new THREE.DirectionalLight(0xb4d4ff, isWhite ? 1.0 : 1.3);
        fill.position.set(-3.5, 2.5, 1.8);
        scene.add(fill);

        // Rim Light (sharp background edge light)
        const rim = new THREE.DirectionalLight(isWhite ? 0xffeed0 : 0x7eb5ff, isWhite ? 2.0 : 3.8);
        rim.position.set(0, 3.8, -4.0);
        scene.add(rim);

        // Soft floor reflection bounce
        const bounce = new THREE.DirectionalLight(0xffe8cc, 0.45);
          ...
          if (child.isMesh) {
            ...
            if (isWhite) {
              child.material = new THREE.MeshStandardMaterial({
                color: new THREE.Color(0xfbf8f3),
                roughness: 0.30,
                metalness: 0.06,
              ...
              child.material = new THREE.MeshStandardMaterial({
                map: origMap,
                color: new THREE.Color(0x32302e),
                roughness: 0.40,
                metalness: 0.28,
                normalMap: origNormal,
        ...
        const scale = 1.52 / maxDim;
        model.scale.set(scale, scale, scale);

        model.position.x = -center.x * scale;
        model.position.y = -box.min.y * scale - 0.72;
      ...
      results['3d_queen_white.png'] = renderPiece(true, 15);
      results['3d_queen_black.png'] = renderPiece(false, 15);

      console.log('Rendered both White and Black 3D Queens. Sending to server...');
      fetch('/save', {
        method: 'POST',
...
server.listen(PORT, () => {
  console.log('Queen render server running on http://localhost:' + PORT);
  ...
  console.log('Launching Edge to render 3D Queen pieces...');
  exec(cmd, (err) => {
